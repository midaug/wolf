...
local function fetch_var(ctx, name, p)
    local prefix = p
    if not prefix then
        prefix = 'WR'
    end
    local key_name = 'http_'..prefix..name
    local val = ctx.var[key_name]
    if not val then
        key_name = 'cookie_'..prefix..name
        val = ctx.var['cookie_'..key_name]
    end
    return val
end

local client_id_names = {"-Client-Id", "_Client_Id"}
local function check_client_id(ctx)
    local clientId
    for _, name in ipairs(client_id_names) do
        clientId = fetch_var(ctx, name)
        if clientId then
            break
        end
    end
    if not clientId then
        return core.response.exit(400, fail_response("clientId is missing"))
    end
    if string.match(clientId, "%w+") == nil then
        return core.response.exit(400, fail_response("clientId is empty"))
    end
    return clientId
end

local ukey_names = {"-ukey", "_ukey"}
local function check_ukey(ctx)
    local ukey
    for _, name in ipairs(ukey_names) do
        ukey = fetch_var(ctx, name)
        if ukey then
            break
        end
    end
    if not ukey then
        return core.response.exit(400, fail_response("ukey is missing"))
    end
    if string.match(ukey, "%w+") == nil then
        return core.response.exit(400, fail_response("ukey is empty"))
    end
    return ukey
end
...
local function fetch_rbac_token(ctx)
    for _, name in ipairs(token_names) do
        local token = fetch_var(ctx, name)
        if token then
            return token
        end
    end

    if ctx.var.arg_token then
        return ngx.unescape_uri(ctx.var.arg_token)
    end
...
local function check_url_permission(server, appid, action, resName, client_ip, wolf_token)
    ...
    if res.status ~= 200 and res.status ~= 401 and res.status ~= 403 then
        return {
            status = 500,
...
function _M.rewrite(conf, ctx)
    ...
    check_client_id(ctx) -- 客户端id不存在时返回400错误
    check_ukey(ctx) -- ukey不存在时返回400错误
    ...
    if rbac_token == nil then
        ...
        return 401, fail_response("Missing token in request")
    end
    ...
    core.log.info("token info: ", core.json.delay_encode(tokenInfo),
        ", err: ", err)
    if err then
        return 401, fail_response('Invalid token: parse failed')
    end
    ...
    if not consumer then
        ...
        return 401, fail_response("Invalid appid in token")
    end
    ...
    if res.status ~= 200 then
        ...
        local errmsg = res.err
        if res.status == 403 then
            errmsg = "Invalid permission"
        end
        core.response.set_header("Content-Type", "application/json")
        return res.status, fail_response(errmsg)
    end
...
local function request_to_wolf_server(method, uri, headers, body)
    ...
    if not res then
        ...
        return core.response.exit(500, fail_response("request to auth-server failed!"))
    end
    core.log.info("request [", request_debug, "] status: ", res.status, ", body: ", res.body)

    local body, err = json.decode(res.body)
    if res.status ~= 200 then
        core.log.error("request [", request_debug, "] failed! status: ", res.status)
        if not body then
            return core.response.exit(res.status, fail_response("request to auth-server failed! no body"))
        end
        return core.response.exit(res.status, fail_response(body.reason, {}))
    end
    if not body then
        core.log.error("request [", request_debug, "] failed! err:", err)
        return core.response.exit(500, fail_response("request to auth-server failed!"))
    end
    local body_json = core.json.delay_encode(body)
    if not body.ok then
        core.log.error("request [", request_debug, "] failed! response body:", body_json)
        return core.response.exit(res.status, fail_response(body.reason, {}))
    end

    core.log.info("request [", request_debug, "] success! response body:", body_json)
    return body
end
...
local function wolf_rbac_login()
    ...
    local ctx = ngx.ctx.api_ctx
    check_client_id(ctx) -- ukey不存在时返回400错误
    check_ukey(ctx) -- ukey不存在时返回400错误
    ...
    local token_exp = body.data.exp

    local rbac_token = create_rbac_token(appid, wolf_token)
    local data = {
        accessToken = rbac_token,
        exp = token_exp
    }
    ...
    core.response.set_header("Content-Type", "application/json")
    core.response.exit(200, success_response(nil, { data = data }))
end
...
local function get_wolf_token(ctx)
    ...
    if rbac_token == nil then
        ...
        core.log.info("no permission to access ",
            core.json.delay_encode(perm_item), ", need login!")
        return core.response.exit(401, fail_response("Missing token in request"))
    end
    ...
    core.log.info("token info: ", core.json.delay_encode(tokenInfo),
        ", err: ", err)
    if err then
        return core.response.exit(401, fail_response('Invalid token: parse failed'))
    end
    return tokenInfo
end
...
local function wolf_rbac_sys_time()
    local ctx = ngx.ctx.api_ctx
    local appid = ctx.var.arg_appid
    if not appid then
        return core.response.exit(400, fail_response("appid is missing"))
    end
    ...
    local uri = consumer.auth_conf.server .. '/wolf/ping'
    local headers = new_headers()
    local body = request_to_wolf_server('GET', uri, headers, {})
...
function _M.api()
    return {
        ...
        {
            methods = { "GET" },
            uri = "/apisix/plugin/wolf-rbac/sys_time",
            handler = wolf_rbac_sys_time,
        },
    }
end
